...
class Recorder:
    ...
    def state(self):
        ...
        state.update({'observation.state.foot_contact':self.arr(torch.linalg.vector_norm(feet,dim=-1)>0).tolist(),'task.goal.position':[sum(e.cfg.approved_profile['goal_x'])/2,sum(e.cfg.approved_profile['goal_y'])/2,0.],'task.progress':[float(center[0])-e._initial_center_x],'task.success':[bool(e.task_success[0])],'task.contact.fl_object':[False],'task.contact.body_object':[False]})
        return state
