...
class JointPositionController:
    ...
    def reset_idx(self, env_ids):
        ...
        desired_FL_positions = self._env._ee_pos_fl_commands[env_ids]  # in projected com frame
        ...
        # for FR swing tracking:
        desired_FR_positions = self._env._ee_pos_fr_commands[env_ids]  # in projected com frame

        d_fr_position = desired_FR_positions.clone()
        d_fr_position[:, 2] -= self._env.base_height[env_ids]
        fr_position_local = torch.matmul(self._env.base_rot_mat_rp_t[env_ids],
                                         d_fr_position.unsqueeze(-1)).squeeze(-1)
        desired_joint_position_fr = self._env.get_motor_angles_from_foot_positions_fr(fr_position_local)

        self._desired_joint_positions[env_ids, 1] = torch.where(idx[:, 1],
                                                                desired_joint_position_fr[:, 0],
                                                                self._desired_joint_positions[env_ids, 1])
        self._desired_joint_positions[env_ids, 5] = torch.where(idx[:, 5],
                                                                desired_joint_position_fr[:, 1],
                                                                self._desired_joint_positions[env_ids, 5])
        self._desired_joint_positions[env_ids, 9] = torch.where(idx[:, 9],
                                                                desired_joint_position_fr[:, 2],
                                                                self._desired_joint_positions[env_ids, 9])
    ...
    def update(self):
        ...
        # for FL swing tracking:
        desired_FL_positions = self._env._ee_pos_fl_commands  # in projected com frame
        ...
        # for FR swing tracking:
        desired_FR_positions = self._env._ee_pos_fr_commands  # in projected com frame

        d_fr_position = desired_FR_positions.clone()
        d_fr_position[:, 2] -= self._env.base_height
        fr_position_local = torch.matmul(self._env.base_rot_mat_rp_t,
                                         d_fr_position.unsqueeze(-1)).squeeze(-1)
        desired_joint_position_fr = self._env.get_motor_angles_from_foot_positions_fr(fr_position_local)

        self._desired_joint_positions[:, 1] = torch.where(idx[:, 1],
                                                          desired_joint_position_fr[:, 0],
                                                          self._desired_joint_positions[:, 1])
        self._desired_joint_positions[:, 5] = torch.where(idx[:, 5],
                                                          desired_joint_position_fr[:, 1],
                                                          self._desired_joint_positions[:, 5])
        self._desired_joint_positions[:, 9] = torch.where(idx[:, 9],
                                                          desired_joint_position_fr[:, 2],
                                                          self._desired_joint_positions[:, 9])
        # phase (0.0):
        idx = torch.logical_and(desired_contact_mode_expand < 0.5,
                                desired_contact_mode_expand > -0.5)
