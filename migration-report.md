...
| M2.3 Kit GUI | **本轮已延期（路径已就绪）** | `run_m2_cartpole_gui_gate.sh` 固定同一 Direct Cartpole/PhysX/Kit，并要求 live `omni.hydra.rtx`/RTX 4090 GPU evidence；真实操作者仍须观察 window、viewport、play/stop、Vulkan、Kit GPU memory 和 RTX 4090，并在 child post-close 后以 TTY 声明。不得以日志或自动化替代。 |
...
2. M2.3 GUI 与 M8 physical-keyboard GUI 已按本轮范围延期；后续由真实操作者在
   `--viz kit` 完成。二者现均有 post-close exit、真人 TTY 声明和人工证据的可审计路径，
   但绝不允许 xdotool、pyautogui、重放或事件注入。
