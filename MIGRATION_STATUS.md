...
| M8 GUI 真实键盘 | **待执行** | 仅接受真实操作者在带 EULA 前缀、非空 `DISPLAY` 的 `--viz kit` 下，用本人实体键盘经当前 Selkies WebRTC 会话实时操作；不使用 xdotool、pyautogui、脚本、重放、自动化或任何注入输入。专用 runner 仅在 Kit child 退出后记录 exit sidecar，随后要求真人 TTY 精确声明并离线验证。 |
